...
public class DatabaseManager {
        ...
        String ddlQueries = String.format("""
                    CREATE TABLE IF NOT EXISTS %s.service (
                                       ...
                                       CREATE TABLE IF NOT EXISTS %s."order" (
                                            ...
                                            CONSTRAINT order_customers_fk FOREIGN KEY (customers) REFERENCES %s.customers(id),
    ...
    public Customer authenticateCustomer(String name, String email, String password) {
        String query = "SELECT id FROM customers WHERE username = ? AND email = ? AND password = ?";
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            if (resultSet.next()) {
                return new Customer(resultSet.getInt("id"), name, email, password);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public Admin authenticateAdmin(String name, String email, String password) {
        String query = "SELECT id FROM admin WHERE username = ? AND email = ? AND password = ?";
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            statement.setString(1, name);
            statement.setString(2, password);
            statement.setString(3, password);
            ResultSet resultSet = statement.executeQuery();
            if (resultSet.next()) {
                return new Admin(resultSet.getInt("id"), name, email, password);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public ObservableList<Customer> customersFetchData() {
        ...
        try (var preparedStatement = connection.prepareStatement(query);
             var resultSet = preparedStatement.executeQuery()) {
            while (resultSet.next()) {
                re.add(new Customer(resultSet.getInt("id"), resultSet.getString("name"), resultSet.getString("email"), resultSet.getString("password")));
            }
        } catch (SQLException e) {
            System.out.println("Ошибка при выполнении запроса: " + e.getMessage());
            // Показываем модальное окно с ошибкой
    ...
    public ObservableList<Order> orderFetchData() {
        ...
        String query = String.format(
                "SELECT o.time AS time, o.date AS date, w.name AS worker_name, " +
                        "c.name AS customer, s.name AS service, p.name AS product " +
                        "FROM %s o " +
                        "LEFT JOIN %s c ON c.id = o.customers " +
    ...
    public Order getOrderById(int orderEditId) {
        ...
        try (var preparedStatement = connection.prepareStatement(query);
            ...
            re.setCustomer(resultSet.getInt("customer"));
            return re;
        } catch (SQLException e) {
            System.out.println("Ошибка при выполнении запроса: " + e.getMessage());
            // Показываем модальное окно с ошибкой
