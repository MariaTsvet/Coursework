...
public class OrderController {
    ...
    public ComboBox<KeyValue> workersList;
    ...
    private void initialize() {
        Platform.runLater(() -> {
            ...
            if (orderEditId == 0) {
                // FIXME: создать новый заказ, забрать его из субд, инициализировать orderEditId
    ...
    public void addProductToOrder() {
        if (primaryDatabaseManager.addProductToOrder(orderEditId, selectedProduct.getId())) {
            updateTableProductsInOrder(this.orderEditId);
        }
    }
    ...
    public void removeProductFromOrder() {
        if (primaryDatabaseManager.removeProductFromOrder(
                orderEditId, selectedProductInOrder.getId())) {
            updateTableProductsInOrder(this.orderEditId);
        }
        ;
    }
    ...
    public void onProductInOrderClicked(MouseEvent event) {
        if (event.getClickCount() == 1) {
            ...
            if (selectedProductInOrder != null) {
                System.out.println("Выбранный продукт: " + selectedProductInOrder.getName());            }
        }
    }
    ...
    public void onWorkerChanged(ActionEvent event) {
        if (primaryDatabaseManager.changeWorkerInOrder(orderEditId, workersList.getValue())) {
            System.out.println("Заказчик был изменён");
        }
        ;
    }
